...
var itemTpl = [
        '<label class="citem weui_cell weui_check_label{{if coupon.is_used == 1 || coupon.is_valid == 0 || choose && coupon.is_able_to_use !== true}} disabled{{/if}}{{if coupon.is_mutex}} conflict{{/if}} " for="coupon{{start + index}}" data-id="{{coupon.coupon_id}}">',
            '<div class="sep"></div>',
...
module.exports = {
    ...
    content: [
            ...
            '{{if list && list.length}}',
                ...
                '<div class="end" data-end = "{{unable_total}}">{{if choose}}没有更多数据了 {{else}}暂无更多可用券 {{if !choose && unable_total > 0 }} <a class="unable">查看失效券<i class="iconf i-arrow-r weui_cell_ft"></i></a>{{/if}}{{/if}}</div>',
            '{{else}}',
                '<div class="empty">您还没有{{type}}{{if !choose && unable_total > 0 }}<a class="unable">查看失效券<i class="iconf i-arrow-r weui_cell_ft"></i></a>{{/if}}</div>',
            '{{/if}}',
    ].join('')
};
